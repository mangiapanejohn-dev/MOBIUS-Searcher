...
**MØBIUS-Searcher 是一个运行在终端里、研究优先的量化机器人。** 它实时监控
Solana 上的 DEX，用 Jupiter 的真实报价给每一条套利路线定价，把真正的交易拼出来
并在主网上模拟执行，然后用数字告诉你：为什么能做、为什么不能做。交易所按
`[venues.*]` 并列配置：目前 Solana 负责执行，OKX 提供行情数据，更多交易所在路上。

> [!WARNING]
> **它不是印钞机。** 在默认配置下，3,091 次真实评估、2,333 次主网模拟里，可执行的
> 机会是 **0**（[测试结果](docs/PAPER_RUN.md)）。默认运行在 **PAPER（模拟盘）**：
> 真实数据、真实模拟，不签名、不发送。CONFIRM 和 LIVE 被明确的开关锁住，至今没有发
> 出过任何一笔交易。本项目的任何内容都不构成投资建议。
